...
import { TrendingUp, FileText, Award, Upload, Briefcase, ArrowRight, Zap, Target, Activity, MessageSquare } from "lucide-react";
...
function ScoreTrendChart({ scores, dates }: { scores: number[], dates: string[] }) {
  if (scores.length === 0) {
    return (
      <div className="h-full w-full flex flex-col items-center justify-center p-6 text-center gap-2">
         <span className="text-3xl font-black text-slate-300">--</span>
         <p className="text-xs text-slate-400 font-medium uppercase tracking-widest">No scores recorded</p>
      </div>
    );
  }
  ...
  if (scores.length === 1) {
    return (
      <div className="h-full w-full flex flex-col items-center justify-center p-6 text-center gap-2">
         <span className="text-4xl font-black text-blue-600 font-mono">{scores[0]}</span>
         <p className="text-xs text-slate-500 font-bold uppercase tracking-widest">First recorded score</p>
         <p className="text-xs text-slate-400 mt-2">Analyze another resume to track your progress.</p>
      </div>
    );
  }
      ...
      <div className="flex flex-col h-full justify-between pt-2">
        <div className="flex items-center gap-3 mb-2">
          ...
          {change !== 0 ? (
            ...
            <span className="text-sm font-bold text-slate-400">No score change</span>
          )}
        </div>
        
        <div className="mt-auto relative w-full h-16 flex items-center px-4">
           {/* Simple 2-point line representation */}
           <div className="absolute top-1/2 left-8 right-8 h-0.5 bg-blue-100 dark:bg-blue-900 -translate-y-1/2 rounded-full" />
           <div className="absolute top-1/2 left-8 right-8 h-0.5 bg-blue-500 -translate-y-1/2 origin-left animate-scale-x rounded-full" />
           
           <div className="w-full flex justify-between items-center relative z-10">
              <div className="flex flex-col items-center gap-2 group cursor-default">
                 <div className="size-3 rounded-full bg-white border-2 border-blue-600 shadow-sm transition-transform group-hover:scale-150" />
                 <span className="text-[10px] text-slate-400 font-bold">{scores[0]}</span>
              </div>
              <div className="flex flex-col items-center gap-2 group cursor-default">
                 <div className="size-3 rounded-full bg-blue-600 border-2 border-white shadow-sm transition-transform group-hover:scale-150" />
                 <span className="text-[10px] text-slate-400 font-bold">{scores[1]}</span>
              </div>
           </div>
        </div>
      </div>
...
export default function Dashboard() {
  ...
  const topSkillsIssue = latestResume.feedback?.skills?.tips?.filter(t => t.type === 'improve') || [];
  
  // Create a realistic AI Insight
  let aiInsight = "Your resume is highly optimized. Consider tailoring it further for specific roles.";
  if (topSkillsIssue.length > 0 && latestResume.feedback?.skills?.score < 70) {
      aiInsight = "Your skills coverage presents the biggest opportunity for improvement. Adding missing industry keywords will boost your ATS match rate.";
  } else if (topIssues.length > 0) {
      aiInsight = `AI Analysis: ${topIssues[0].tip}`;
  } else if (latestScore < 60) {
      aiInsight = "Your resume structure and content need significant revisions to pass modern ATS filters.";
  }
  ...
  return (
    <PageTransition className="flex flex-col gap-8 max-w-7xl mx-auto pb-16 relative z-10 w-full px-4 lg:px-8">
      ...
      {/* Row 1: Resume Health + Metrics + Quick Actions */}
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6 lg:gap-8">
        
        {/* Left Column (Primary Health) */}
        <div className="xl:col-span-2 flex flex-col gap-6 lg:gap-8">
          <ScrollReveal delay={0.1} direction="up" distance={30} className="h-full">
            <div className="bg-white/60 dark:bg-slate-900/60 backdrop-blur-2xl border border-border/60 rounded-[2rem] p-6 lg:p-10 shadow-[0_20px_40px_-15px_rgba(0,0,0,0.05)] dark:shadow-[0_20px_40px_-15px_rgba(0,0,0,0.2)] flex flex-col md:flex-row items-center md:items-start gap-8 lg:gap-12 relative overflow-hidden group h-full">
        ...
        <div className="flex flex-col gap-6 lg:gap-8">
          <ScrollReveal delay={0.15} direction="up" distance={30}>
            <div className="grid grid-cols-3 gap-4">
              <TiltCard tiltAmount={5}>
                <div className="bg-white/50 dark:bg-slate-900/50 backdrop-blur-md border border-border/50 rounded-2xl p-4 flex flex-col gap-1 shadow-sm hover:border-blue-600/30 transition-colors h-full">
                  <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Analyzed</span>
                  <span className="text-2xl font-black font-mono text-foreground mt-auto">{totalAnalyzed}</span>
                </div>
              </TiltCard>
              <TiltCard tiltAmount={5}>
                <div className="bg-white/50 dark:bg-slate-900/50 backdrop-blur-md border border-border/50 rounded-2xl p-4 flex flex-col gap-1 shadow-sm hover:border-blue-600/30 transition-colors h-full">
                  <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Average</span>
                  <span className="text-2xl font-black font-mono mt-auto" style={{ color: avgScore > 0 ? scoreColor(avgScore) : "inherit" }}>
                    {avgScore || "—"}
                  </span>
                </div>
              </TiltCard>
              <TiltCard tiltAmount={5}>
                <div className="bg-white/50 dark:bg-slate-900/50 backdrop-blur-md border border-border/50 rounded-2xl p-4 flex flex-col gap-1 shadow-sm hover:border-blue-600/30 transition-colors h-full">
                  <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Highest</span>
                  <span className="text-2xl font-black font-mono mt-auto" style={{ color: highestScore > 0 ? scoreColor(highestScore) : "inherit" }}>
                    {highestScore || "—"}
                  </span>
                </div>
              </TiltCard>
          ...
          <ScrollReveal delay={0.25} direction="up" distance={30} className="flex flex-col gap-4 h-full">
            ...
            <div className="flex flex-col gap-3">
              ...
              <Link to="/job-match" className="bg-white/50 dark:bg-slate-900/50 backdrop-blur-md border border-border/50 hover:border-border hover:bg-white dark:hover:bg-slate-900 rounded-2xl p-4 flex items-center justify-between transition-all group shadow-sm hover:shadow-md">
                <div className="flex items-center gap-3">
                   ...
                   <span className="font-semibold text-sm text-slate-700 dark:text-slate-300 group-hover:text-foreground transition-colors">Target Job Match</span>
                </div>
              ...
              <Link to="/copilot" className="bg-white/50 dark:bg-slate-900/50 backdrop-blur-md border border-border/50 hover:border-border hover:bg-white dark:hover:bg-slate-900 rounded-2xl p-4 flex items-center justify-between transition-all group shadow-sm hover:shadow-md">
                <div className="flex items-center gap-3">
                   <div className="p-2 rounded-xl bg-slate-200 dark:bg-slate-800 text-slate-600 dark:text-slate-400 group-hover:text-blue-600 transition-colors"><MessageSquare size={18} /></div>
                   <span className="font-semibold text-sm text-slate-700 dark:text-slate-300 group-hover:text-foreground transition-colors">AI Copilot</span>
                </div>
                <ArrowRight size={18} className="opacity-0 group-hover:opacity-100 text-blue-600 transition-all -translate-x-2 group-hover:translate-x-0" />
              </Link>
            </div>
          </ScrollReveal>
        </div>
      ...
      {/* Row 2: Recent Analyses & Score Trend */}
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6 lg:gap-8">
        <ScrollReveal delay={0.2} direction="up" distance={30} className="flex flex-col gap-4">
          ...
          {recentResumes.length > 0 ? (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="font-bold text-foreground text-sm truncate group-hover:text-blue-600 transition-colors">
                      {resume.jobTitle || "Untitled Analysis"}
                    </p>
            ...
            <div className="bg-white/50 dark:bg-slate-900/50 backdrop-blur-md border border-border/50 rounded-2xl p-8 flex flex-col items-center justify-center text-center gap-4 h-[240px]">
               <FileText size={32} className="text-slate-300" />
               <div>
                 <p className="font-bold text-slate-700 dark:text-slate-300">No Analyses Yet</p>
                 <p className="text-sm text-slate-500 mt-1">Analyze your first resume to start building your resume intelligence.</p>
               </div>
               <Link to="/upload" className="px-4 py-2 bg-blue-600/10 text-blue-600 font-bold rounded-lg text-sm mt-2 hover:bg-blue-600/20 transition-colors">
                 Analyze Resume
               </Link>
            </div>
          )}
        ...
        <ScrollReveal delay={0.3} direction="up" distance={30} className="flex flex-col gap-4">
           ...
           <div className="bg-white/50 dark:bg-slate-900/50 backdrop-blur-md border border-border/50 rounded-2xl p-6 h-full min-h-[240px] flex flex-col hover:shadow-md transition-shadow">
             <ScoreTrendChart scores={scoreTrendData} dates={scoreTrendDates} />
           </div>
        </ScrollReveal>
      </div>

      {/* Row 3: AI Insight */}
      <ScrollReveal delay={0.35} direction="up" distance={30} className="w-full">
        <div className="bg-gradient-to-br from-slate-900 to-slate-800 rounded-[2rem] p-6 lg:p-8 relative overflow-hidden shadow-2xl group flex flex-col md:flex-row items-center md:items-start gap-6 lg:gap-8">
          <div className="absolute top-0 right-0 w-64 h-64 bg-blue-500/20 blur-[80px] pointer-events-none transition-all group-hover:bg-blue-500/30" />
          
          <div className="shrink-0 size-16 md:size-20 relative">
          ...
          <div className="flex flex-col gap-3 flex-1 text-center md:text-left relative z-10">
            <h4 className="text-sm font-bold text-blue-400 uppercase tracking-widest flex items-center justify-center md:justify-start gap-1.5">
              <Zap size={14} /> AI Insight
            </h4>
            <p className="text-white text-base md:text-lg leading-relaxed opacity-90 italic">
              "{aiInsight}"
            </p>
          </div>
          
          <div className="shrink-0 relative z-10">
             <Link to={`/resume/${latestResume.id}`} className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-white/10 text-white font-semibold text-sm hover:bg-white/20 transition-colors border border-white/10">
